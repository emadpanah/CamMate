...
bool Recorder::fileExists(const char* pathJson){
  return SPIFFS.exists(pathJson);
}

bool Recorder::clearFile(const char* path){
  if (SPIFFS.exists(path)) return SPIFFS.remove(path);
  return true;
}
...
void Recorder::_closeWrite(){
  if (_wf) _wf.close();
}

void Recorder::pushLive(float x, float y, UIMode mode, float diam, SpeedMode spd){
  _lx = clamp11f(x);
  _ly = clamp11f(y);
  _lm = mode;
  _ld = clamp01f(diam);
...
bool Recorder::stopRecording(){
  ...
  if (m) {
    m.printf("{\"frames\":%u,\"duration_ms\":%u}\n", _framesRecorded, _lastT);
    m.close();
  }
  return true;
}
...
bool Recorder::_loadFile(const char* pathJson){
  ...
  while (f.available()) {
    ...
    RecFrame fr{};
    int ti = line.indexOf("\"t\":");    int xi = line.indexOf("\"x\":");
    int yi = line.indexOf("\"y\":");    int mi = line.indexOf("\"mode\":");
    int di = line.indexOf("\"diam\":"); int si = line.indexOf("\"speed\":");
    ...
    if (_frames.size() > 30000) break; // safety
  }
  f.close();
...
void Recorder::tick(uint32_t nowMs, void (*onApply)(const RecFrame&)){
  ...
  if (_state == REC_RECORDING) {
    if ((int32_t)(nowMs - _nextSample) >= 0) {
      ...
      if (_wf) {
        _wf.printf("{\"t\":%u,\"x\":%.3f,\"y\":%.3f,\"mode\":%u,\"diam\":%.2f,\"speed\":%u}\n",
                   t, _lx, _ly, (unsigned)_lm, _ld, (unsigned)_ls);
        _wf.flush();
        _framesRecorded++;
        _lastT = t;
      }
